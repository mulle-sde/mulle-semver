# Mulle Tool Documentation Process Guide

## Overview

This document outlines the systematic process for creating comprehensive command documentation for mulle tools. Based on practical experience with documentation creation, it provides clear guidelines for what to write, what to avoid, and the step-by-step process to follow.
...
- **MANDATORY: Include hidden/utility commands** - Commands like `etc-dir`, `project-dir`, `version`, etc. are often overlooked but must be documented
- **Identify command categories** (core operations, pattern management, file matching, system, utility)
...
- Core Operations commands (`init`, `list`, `filename`)
- Pattern Management commands (`patternfile add`, `patternfile list`, `patternfile remove`)
- File Matching commands (`list`, `filename`)
- Configuration commands (`patternfile install`)
- System information (`patternfile cat`)

### Medium Priority Commands
- Advanced pattern operations (`patternfile install`, `patternfile cat`)
- Integration features (`list --match-filter`)
...
#### [Core Operations]
- [ ] `{TOOL_NAME} init <args>` : Initialize patternfiles for file matching
- [ ] `{TOOL_NAME} list <args>` : List files matching patternfiles
- [ ] `{TOOL_NAME} filename <args>` : Test filename against patternfiles

#### [Pattern Management]
- [ ] `{TOOL_NAME} patternfile add <args>` : Add a new patternfile
- [ ] `{TOOL_NAME} patternfile list <args>` : List installed patternfiles
- [ ] `{TOOL_NAME} patternfile remove <args>` : Remove a patternfile
- [ ] `{TOOL_NAME} patternfile cat <args>` : Display patternfile contents

#### [Configuration & Info]
- [ ] `{TOOL_NAME} patternfile install <args>` : Install patternfile to ignore.d or match.d
- [ ] `{TOOL_NAME} --help <args>` : Show help information
